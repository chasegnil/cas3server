...
import com.github.chasegnil.cas3.exception.BadImgVerifyCodeAuthenticationException;
import com.github.chasegnil.cas3.exception.NullImgVerifyCodeAuthenticationException;
...
/**
 * 在原生的AuthenticationViaFormAction基础上加了校验码判断
 */
public class AuthenticationViaFormAction {
    ...
    public final String validatorCode(final RequestContext context, final Credentials credentials, final MessageContext messageContext) throws Exception {
        // Validate imgverifycode
        final HttpServletRequest request = WebUtils.getHttpServletRequest(context);
        HttpSession session = request.getSession();
        UsernamePasswordCredentials upc = (UsernamePasswordCredentials) credentials;
        String submitAuthcode = upc.getImgverifycode();
        if (!StringUtils.hasText(submitAuthcode)) {
            populateErrorsInstance(new NullImgVerifyCodeAuthenticationException(), messageContext);
            return "error";
        }
        if (org.apache.commons.lang3.StringUtils.equalsIgnoreCase((String)session.getAttribute(Constants.KAPTCHA_SESSION_KEY),submitAuthcode)){
            return "success";
        }
